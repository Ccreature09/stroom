...
async function requireLayoutContext(warehouseId: number) {
  ...
  if (employee.canModifyLocations !== true) {
    throw new Error("You do not have permission to modify the warehouse layout.");
  }
...
export async function createHall(formData: FormData) {
  ...
  redirect(`/dashboard/warehouses/${warehouseId}/layout-designer?hall=${hall.hallId}`);
}
...
export async function createZoneType(formData: FormData): Promise<ActionResult> {
  const warehouseId = parsePositiveInt(formData.get("warehouseId"));
  ...
  try {
    await db.insert(zoneTypes).values({
      ...
      storagePermanence: String(formData.get("storagePermanence") ?? "PERMANENT"),
    });
  } catch {
...
export async function createLocation(formData: FormData): Promise<ActionResult> {
  ...
  if (physicalX === null || physicalY === null || !physicalWidthMm || !physicalLengthMm) {
    return { error: "Location geometry is invalid -- redraw it on the canvas." };
  }
  ...
  if ([aisle, bay, level, heightMm, maxWeightKg].some((v) => Number.isNaN(v))) {
    return { error: "Aisle, bay, level, height, and max weight must be whole numbers." };
  }
...
export async function updateLocationDetails(formData: FormData): Promise<ActionResult> {
  const warehouseId = parsePositiveInt(formData.get("warehouseId"));
  const locationId = parsePositiveInt(formData.get("locationId"));
  if (!warehouseId || !locationId) return { error: "Missing warehouse or location." };
  ...
  try {
    await db
      .update(locations)
      .set({ locationCode, zoneId, aisle, bay, level, heightMm, maxWeightKg, isBlocked, updatedAt: new Date().toISOString() })
...
export async function updateLocationGeometry(
  warehouseId: number,
  locationId: number,
  geometry: { physicalX: number; physicalY: number; physicalWidthMm: number; physicalLengthMm: number; rotationDegrees: number },
  ...
  const normalizedRotation = ((Math.round(geometry.rotationDegrees) % 360) + 360) % 360;
...
export async function deleteLocation(formData: FormData): Promise<ActionResult> {
  ...
  await db.delete(locations).where(and(eq(locations.locationId, locationId), eq(locations.warehouseId, warehouseId)));
