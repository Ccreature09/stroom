...
export async function createItem(formData: FormData) {
  ...
  const lengthCm = formData.get("lengthCm") ? String(formData.get("lengthCm")) : "0.00";
  const widthCm = formData.get("widthCm") ? String(formData.get("widthCm")) : "0.00";
  const heightCm = formData.get("heightCm") ? String(formData.get("heightCm")) : "0.00";
  const weightKg = formData.get("weightKg") ? String(formData.get("weightKg")) : "0.000";

  // Integer fields
  const shelfLifeDays = formData.get("shelfLifeDays") ? Number(formData.get("shelfLifeDays")) : null;
  const minStockLevel = formData.get("minStockLevel") ? Number(formData.get("minStockLevel")) : 0;
