    ...
    <main className="flex-1 bg-slate-50 px-5 py-10 sm:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="rounded-2xl border border-slate-200 bg-white px-6 py-6 shadow-sm sm:px-8">
          ...
          <h1 className="mt-3 text-3xl font-bold tracking-[-0.04em] text-slate-950">Warehouses</h1>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-bold tracking-[-0.03em] text-slate-950">Warehouse modules</h2>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {warehouseRows.map((warehouse, index) => (
                <Link
                  ...
                  className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-300 hover:shadow-md"
                >
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    {[warehouse.street, warehouse.city, warehouse.postalCode, warehouse.country]
                      .filter(Boolean)
                      .join(", ") || "No address yet"}
                  </p>
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                        warehouse.isActive ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                      }`}
                    >
                      {warehouse.isActive ? "Active" : "Inactive"}
                    </span>
                    <span className="text-sm font-semibold text-teal-700">Open →</span>
                  </div>
                </Link>
              ))}
            </div>
