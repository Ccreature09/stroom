import { and, asc, desc, eq, inArray, isNull, or } from "drizzle-orm";
import { db } from "@/lib/db";
import {
  employees,
  items,
  locations,
  positionTypes,
  replenishmentTasks,
  taskStatuses,
  tasks,
  taskTypes,
} from "@/drizzle/schema";
import { requireInternalManagerAccess } from "@/lib/warehouse-access";
import { getTaskLookups } from "@/lib/inbound/task-lookups";
import { loadReplenishmentNeeds } from "@/lib/internal/replenishment-server";
import { listWarehouseDepartments, loadTaskDepartments } from "@/lib/tasks/routing";
import { DynamicBreadcrumb } from "@/components/layout/dynamic-breadcrumb";
import { ArrowDownUp } from "lucide-react";
import { ReplenishmentSuggestions, ReplenishmentTaskActions } from "./replen-controls";
import { TaskRouting } from "../../task-routing";

const statusStyles: Record<string, string> = {
  PENDING: "bg-slate-100 text-slate-600 border-slate-200",
  IN_PROGRESS: "bg-blue-50 text-blue-700 border-blue-200",
  COMPLETED: "bg-emerald-50 text-emerald-700 border-emerald-200",
  CANCELLED: "bg-red-50 text-red-700 border-red-200",
};

export default async function ReplenishmentPage({
  params,
}: {
  params: Promise<{ warehouseId: string }>;
}) {
  const { warehouseId } = await params;
  const { employee, warehouseId: parsedWarehouseId } =
    await requireInternalManagerAccess(warehouseId);

  await getTaskLookups();

  const [needs, openTasks, employeeOptions] = await Promise.all([
    loadReplenishmentNeeds(parsedWarehouseId, employee.organizationId),
    db
      .select({
        taskId: tasks.taskId,
        statusCode: taskStatuses.code,
        assignedEmployeeId: tasks.assignedEmployeeId,
        assigneeFirstName: employees.firstName,
        assigneeLastName: employees.lastName,
        quantity: replenishmentTasks.quantity,
        batchNumber: replenishmentTasks.batchNumber,
        lotNumber: replenishmentTasks.lotNumber,
        sourceLocationId: replenishmentTasks.sourceLocationId,
        destinationLocationId: replenishmentTasks.destinationLocationId,
        sku: items.sku,
        itemName: items.name,
      })
      .from(replenishmentTasks)
      .innerJoin(tasks, eq(replenishmentTasks.taskId, tasks.taskId))
      .innerJoin(taskStatuses, eq(tasks.statusId, taskStatuses.statusId))
      .innerJoin(taskTypes, eq(tasks.taskTypeId, taskTypes.taskTypeId))
      .leftJoin(items, eq(replenishmentTasks.itemId, items.itemId))
      .leftJoin(employees, eq(tasks.assignedEmployeeId, employees.employeeId))
      .where(
        and(
          eq(tasks.warehouseId, parsedWarehouseId),
          eq(taskTypes.code, "REPLENISHMENT"),
          inArray(taskStatuses.code, ["PENDING", "IN_PROGRESS"]),
        ),
      )
      .orderBy(desc(tasks.createdAt))
      .limit(200),
    db
      .select({
        employeeId: employees.employeeId,
        firstName: employees.firstName,
        lastName: employees.lastName,
      })
      .from(employees)
      .innerJoin(positionTypes, eq(employees.positionId, positionTypes.positionId))
      .where(
        and(
          eq(employees.organizationId, employee.organizationId),
          eq(employees.isActive, true),
          eq(positionTypes.canReplenish, true),
          or(
            eq(employees.primaryWarehouseId, parsedWarehouseId),
            eq(employees.currentWarehouseId, parsedWarehouseId),
            and(
              isNull(employees.primaryWarehouseId),
              isNull(employees.currentWarehouseId),
            ),
          ),
        ),
      )
      .orderBy(asc(employees.firstName)),
  ]);

  // Codes for the open-task table -- a replenishment references two
  // locations, so joining `locations` twice would need two aliases.
  const locationRows = await db
    .select({ locationId: locations.locationId, locationCode: locations.locationCode })
    .from(locations)
    .where(eq(locations.warehouseId, parsedWarehouseId));
  const codeById = new Map(locationRows.map((l) => [l.locationId, l.locationCode]));

  const [departmentOptions, taskDepartments] = await Promise.all([
    listWarehouseDepartments(parsedWarehouseId),
    loadTaskDepartments(openTasks.map((r) => r.taskId)),
  ]);

  const canReplenish = employee.canReplenish === true;
  const canAssign = employee.canAssignTasks === true;

  return (
    <main className="flex-1 space-y-6 p-8">
      <div>
        <DynamicBreadcrumb />
      </div>

      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Replenishment</h1>
        <p className="text-sm text-slate-500">
          Pick faces running below their minimum, and the reserve stock that
          can top them up.
        </p>
      </div>

      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700">
          Suggested ({needs.length})
        </h2>
        <ReplenishmentSuggestions
          warehouseId={parsedWarehouseId}
          needs={needs.map((n) => ({
            itemId: n.itemId,
            sku: n.sku,
            itemName: n.itemName,
            pickFaceQuantity: n.pickFaceQuantity,
            minStockLevel: n.minStockLevel,
            deficit: n.deficit,
            destinationLocationId: n.destinationLocationId,
            destinationLocationCode: n.destinationLocationCode,
            shortfall: n.plan.shortfall,
            picks: n.plan.picks.map((p) => ({
              locationId: p.candidate.locationId,
              locationCode: p.candidate.locationCode,
              batchNumber: p.candidate.batchNumber,
              lotNumber: p.candidate.lotNumber,
              expiryDate: p.candidate.expiryDate,
              quantity: p.quantity,
            })),
          }))}
          canReplenish={canReplenish}
          employeeOptions={employeeOptions}
          departmentOptions={departmentOptions}
        />
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700">
          Open moves ({openTasks.length})
        </h2>
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 text-xs font-semibold text-slate-700 uppercase tracking-wider border-b border-slate-200">
              <tr>
                <th className="px-6 py-3">Item</th>
                <th className="px-6 py-3">From → To</th>
                <th className="px-6 py-3">Batch / Lot</th>
                <th className="px-6 py-3 text-right">Qty</th>
                <th className="px-6 py-3">Status</th>
                <th className="px-6 py-3">Routed To</th>
                <th className="px-6 py-3">Assigned To</th>
                <th className="px-6 py-3 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {openTasks.map((row) => (
                <tr key={row.taskId} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    <div className="font-mono text-xs font-bold text-slate-900">
                      {row.sku ?? "-"}
                    </div>
                    <div className="text-xs text-slate-500">{row.itemName ?? ""}</div>
                  </td>
                  <td className="px-6 py-4 font-mono text-xs text-slate-900">
                    {codeById.get(row.sourceLocationId) ?? "?"}{" "}
                    <span className="text-slate-400">→</span>{" "}
                    <span className="font-bold text-teal-700">
                      {codeById.get(row.destinationLocationId) ?? "?"}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-mono text-xs text-slate-500">
                    {row.batchNumber ?? "-"}
                    {row.lotNumber ? ` / ${row.lotNumber}` : ""}
                  </td>
                  <td className="px-6 py-4 text-right font-mono text-sm font-semibold text-slate-900">
                    {row.quantity}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                        statusStyles[row.statusCode ?? ""] ??
                        "bg-slate-100 text-slate-700 border-slate-200"
                      }`}
                    >
                      {(row.statusCode ?? "").replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <TaskRouting
                      warehouseId={parsedWarehouseId}
                      taskId={row.taskId}
                      departments={taskDepartments.get(row.taskId) ?? []}
                      options={departmentOptions}
                      canEdit={canAssign}
                    />
                  </td>
                  <td className="px-6 py-4 text-xs text-slate-600">
                    {row.assignedEmployeeId ? (
                      [row.assigneeFirstName, row.assigneeLastName]
                        .filter(Boolean)
                        .join(" ") || `#${row.assignedEmployeeId}`
                    ) : (
                      <span className="text-slate-400">Unassigned</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <ReplenishmentTaskActions
                      warehouseId={parsedWarehouseId}
                      taskId={row.taskId}
                      statusCode={row.statusCode ?? "PENDING"}
                      canReplenish={canReplenish}
                      canAssign={canAssign}
                      employeeOptions={employeeOptions}
                    />
                  </td>
                </tr>
              ))}
              {openTasks.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-slate-500">
                    <div className="flex flex-col items-center gap-2">
                      <ArrowDownUp className="h-6 w-6 text-slate-300" />
                      No replenishment moves in progress.
                    </div>
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </section>
    </main>
  );
}
