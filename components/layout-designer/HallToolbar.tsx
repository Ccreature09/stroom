    ...
    <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white/70 px-4 py-3">
      ...
      <div className="flex items-center gap-1 rounded-lg border border-slate-300 bg-white p-1">
        <button
          ...
          className={`rounded-md px-3 py-1.5 text-sm font-medium transition ${tool === "select" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
        >
          Select
        </button>
        <button
          onClick={() => onToolChange("draw")}
          className={`rounded-md px-3 py-1.5 text-sm font-medium transition ${tool === "draw" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
        >
          Add location
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {zoneTypes.map((z) => (
          <span key={z.zoneId} className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-600">
            <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: cssColorForZone(z.zoneId) }} />
            {z.name}
          </span>
        ))}
        <button
...
function NewHallDialog({ warehouseId, onClose }: { warehouseId: number; onClose: () => void }) {
  // Plain server-action form (not wrapped in startTransition/try-catch):
  // createHall ends in redirect(), and Next's redirect() throws a special
  // signal that a generic client-side try/catch would swallow and misreport
  // as an error instead of letting the framework navigate.
  return (
    <DialogShell title="New hall" onClose={onClose}>
      <form action={createHall} className="mt-4 space-y-3">
        ...
        <div>
          ...
          <input id="name" name="name" required autoFocus placeholder="Hall B - Cold Storage" className={inputClass} />
        </div>
    ...
    <DialogShell title="New zone" onClose={onClose}>
      <form onSubmit={handleSubmit} className="mt-4 space-y-3">
        ...
        <div>
          ...
          <input id="zone-name" name="name" required autoFocus placeholder="BULK" className={inputClass} />
        </div>
        <div>
