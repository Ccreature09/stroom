...
/**
 * Shared route-computation shell, called by both the designer's design-time
 * preview and the live map's congestion-aware preview.
 *
 * The two callers differ only in *what they pass in*: the designer never
 * supplies `congestionMultipliers`, so its routes are exactly the ones the
 * layout implies, with no dependency on live data. The live map fetches
 * current multipliers first and passes them through. Neither caller
 * duplicates the ~150 lines of "load access points, build the graph,
 * sequence stops, run A*, format the response" in between.
 */
...
/**
 * In-process cache of the compiled BASE routing graph (no congestion
 * applied). Keyed by (hall, layout version, graph epoch) because that triple
 * is exactly what invalidates it: republishing moves the version, a
 * blockage moves the epoch. Shared across both callers, so routing on the
 * same hall from the designer and the live map in the same process does not
 * build the graph twice.
 */
...
export async function computeRoutePreview(
  ...
  options: {
    mheTypeId?: number | null;
    persist?: boolean;
    taskId?: string;
    /** Per-edge impedance multiplier, e.g. from getCongestionMultipliers.
     *  Omitted entirely by the designer -- that is what keeps design-time
     *  previews free of any dependency on live traffic data. */
    congestionMultipliers?: Map<number, number>;
  } = {},
  ...
  const graph = options.congestionMultipliers
    ? withImpedanceOverrides(baseGraph, options.congestionMultipliers)
    : baseGraph;
